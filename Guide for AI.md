...
str_prev_char(s, byte_pos) -> int
str_next_char(s, byte_pos) -> int
...
Runtime-backed mutable singleton storage.

---

# 3.14 Process Execution

```aial
process::run(cmd) -> string
process::run_status(cmd) -> int
```

Executes a shell command and returns stdout.

`run_status` returns a heap block `[stdout_ptr, exit_code]`.
